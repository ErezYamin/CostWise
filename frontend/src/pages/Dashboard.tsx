  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16 }}>
        <KpiCard title="Total Revenue Today" value="$8,430" trend={12} icon={<DollarSign size={18} />} color="#3B82F6" />
        <KpiCard title="Total Orders Today" value="214" trend={8} icon={<ShoppingCart size={18} />} color="#10B981" />
        <KpiCard title="Average Order Value" value="$39.40" trend={-2} icon={<TrendingUp size={18} />} color="#8B5CF6" />
        <KpiCard title="Active Employees" value="18" trend={0} icon={<Users size={18} />} color="#F59E0B" />
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 16 }}>
        ...
        <div style={cardStyle}>
          ...
          <ResponsiveContainer width="100%" height={220}>
            <PieChart>
              <Pie data={ordersData} cx="50%" cy="50%" innerRadius={60} outerRadius={90} dataKey="value">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 16 }}>
            {ordersData.map((item, index) => (
              <div key={index} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <span style={{ flex: 1, fontSize: 13, color: '#8899BB' }}>{item.name}</span>
                <span style={{ fontSize: 13, fontWeight: 600, color: '#EFF6FF' }}>{item.value}%</span>
              </div>
            ))}
          </div>
